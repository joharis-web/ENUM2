...
<head>
    ...
    @push('scripts')
<script>
    function incrementQuantity(pesanId) {
        let input = document.getElementById(`quantityInput_${pesanId}`);
        input.value = parseInt(input.value) + 1;
        updateQuantity(pesanId, input.value);
    }

    function decrementQuantity(pesanId) {
        let input = document.getElementById(`quantityInput_${pesanId}`);
    ...
    function updateQuantity(pesanId, newQuantity) {
        fetch(`/update-pesan/${pesanId}/${newQuantity}`, {
            method: 'PATCH',
            headers: {
            ...
            if (response.ok) {
                // Jika pembaruan berhasil, perbarui total dan subtotal di tampilan
                updateTotals(); // Panggil fungsi untuk meng-update total dan subtotal
            }
        }).catch(error => {
    ...
    function updateTotals() {
        // Lakukan request baru untuk mengambil nilai total dan subtotal yang baru
        fetch(`/get-totals`, {
            method: 'GET',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
          ...
          .then(data => {
              // Update nilai total dan subtotal di tampilan dengan nilai yang baru
...
document.getElementById('payment_method').addEventListener('change', function() {
    const paymentProofField = document.getElementById('proofField');
        ...
        <nav class="container px-6 py-8 mx-auto md:flex md:justify-between md:items-center">
            ...
            <div :class="isOpen ? 'flex' : 'hidden'"
                ...
                <a class="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-500 hover:text-green-400"
                    ...
                    <a href="{{ route('pesan.index')}}" align="center">
                        
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-green-400" fill="none" viewBox="0 0 24 20"
                          stroke="currentColor">
